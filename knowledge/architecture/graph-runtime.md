...
HTTP sessions are scoped to a daemon lifetime. Initialization without a session header creates an opaque ID from 32 bytes of operating-system randomness. A supplied unknown or lost session ID receives HTTP 404 with JSON-RPC error `-32002`, including initialization and notifications; a missing required header receives HTTP 400. Clients recover by initializing again without the old header, then using the returned ID. Existing-session reinitialization remains supported. Entropy failure or an ID collision rejects initialization with HTTP 500 and error `-32603` without changing session state. Session rejection precedes graph admission, so busy graph execution cannot mask a lost session. This contract does not add automatic client retries, persistent sessions, expiry, capacity limits, or DELETE support.
