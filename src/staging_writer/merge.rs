...
pub(super) fn merge_node_value(existing: &mut NodeStagedRow, incoming: NodeStagedRow) {
    ...
    merge_optional_string(&mut existing.grammar_version, incoming.grammar_version);
...
pub(super) fn merge_edge_value(existing: &mut EdgeStagedRow, incoming: EdgeStagedRow) {
    ...
    merge_optional_string(&mut existing.field_name, incoming.field_name);
    merge_optional_i64(&mut existing.child_index, incoming.child_index);
    merge_optional_i64(&mut existing.line_start, incoming.line_start);
    merge_optional_i64(&mut existing.line_end, incoming.line_end);
    merge_optional_i64(&mut existing.byte_start, incoming.byte_start);
    merge_optional_i64(&mut existing.byte_end, incoming.byte_end);
