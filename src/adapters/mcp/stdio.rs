...
pub(crate) fn serve_mcp_stdio<R: BufRead, W: Write>(
    ...
    options.api = Some(start_configured_api(&options)?);
...
pub(in crate::adapters) fn read_mcp_message<R: BufRead, W: Write>(
    input: &mut R,
    output: &mut W,
...
pub(in crate::adapters) fn write_mcp_message<W: Write>(
    output: &mut W,
