...
use crate::product_cli::{constants::MAX_HTTP_BODY_BYTES, install::McpHttpState};
use serde_json::json;
use std::{
...
pub(in crate::product_cli) fn serve_mcp_http(options: &McpHttpOptions) -> Result<(), String> {
    let listener = options.bind_listener()?;
    serve_mcp_http_listener(options, listener, None)
}

pub(in crate::product_cli) fn serve_mcp_http_listener(
    options: &McpHttpOptions,
...
pub(in crate::product_cli) fn handle_mcp_http_stream(
    options: &McpHttpOptions,
...
pub(in crate::product_cli) fn handle_mcp_http_request(
    options: &McpHttpOptions,
    state: &mut McpHttpState,
    request: HttpRequest,
) -> HttpResponse {
...
pub(in crate::product_cli) fn valid_http_origin(origin: Option<&str>) -> bool {
...
pub(in crate::product_cli) fn http_origin_host(origin: &str) -> Option<String> {
...
pub(in crate::product_cli) fn read_http_request(
    stream: &mut TcpStream,
...
pub(in crate::product_cli) fn find_header_end(buffer: &[u8]) -> Option<usize> {
    buffer.windows(4).position(|window| window == b"\r\n\r\n")
}

pub(in crate::product_cli) fn write_http_json(
    stream: &mut TcpStream,
    status: u16,
...
pub(in crate::product_cli) fn http_reason(status: u16) -> &'static str {
    match status {
        200 => "OK",
        202 => "Accepted",
        400 => "Bad Request",
...
#[derive(Debug)]
pub(in crate::product_cli) struct HttpRequest {
    pub(in crate::product_cli) method: String,
    pub(in crate::product_cli) path: String,
    pub(in crate::product_cli) headers: BTreeMap<String, String>,
    pub(in crate::product_cli) body: Vec<u8>,
    pub(in crate::product_cli) body_too_large: bool,
}

impl HttpRequest {
    pub(in crate::product_cli) fn header(&self, name: &str) -> Option<&str> {
        self.headers
...
#[derive(Debug)]
pub(in crate::product_cli) struct HttpResponse {
    pub(in crate::product_cli) status: u16,
    pub(in crate::product_cli) payload: serde_json::Value,
    pub(in crate::product_cli) headers: Vec<(String, String)>,
}

impl HttpResponse {
    pub(in crate::product_cli) fn json(status: u16, payload: serde_json::Value) -> Self {
        Self {
            status,
            payload,
            headers: Vec::new(),
...
pub(in crate::product_cli) fn is_local_host(host: &str) -> bool {
