...
pub(in crate::product_cli) fn watch_max_wait(debounce_ms: u64) -> Duration {
    Duration::from_secs(5).max(Duration::from_millis(debounce_ms.saturating_mul(10)))
}

pub(in crate::product_cli) fn watch_matches_any_pattern(path: &str, patterns: &[String]) -> bool {
    patterns
        .iter()
...
pub(in crate::product_cli) fn watch_glob_matches(path: &str, pattern: &str) -> bool {
...
pub(in crate::product_cli) fn watch_normalize_pattern(pattern: &str) -> String {
    pattern
        .trim()
        .trim_start_matches("./")
        .replace('\\', "/")
...
pub(in crate::product_cli) fn watch_wildcard_match(text: &str, pattern: &str) -> bool {
