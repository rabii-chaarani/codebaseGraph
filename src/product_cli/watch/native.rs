...
use crate::product_cli::materialize::{materialize, MaterializeOptions};
...
pub(in crate::product_cli) fn start_native_watcher(
    source_root: &Path,
...
pub(in crate::product_cli) fn run_native_watch<W: Write>(
    stdout: &mut W,
...
pub(in crate::product_cli) fn probe_native_watcher(
    source_root: &Path,
    filter: &WatchEventFilter,
    rx: &Receiver<WatchMessage>,
...
pub(in crate::product_cli) fn watch_probe_timeout() -> Duration {
...
pub(in crate::product_cli) fn watch_probe_skip_write() -> bool {
    env::var("CODEBASE_GRAPH_WATCH_PROBE_SKIP_WRITE").is_ok_and(|value| value == "1")
}

pub(in crate::product_cli) fn watch_event_is_under_dir(
    event: &Event,
    directory: &Path,
    source_root: &Path,
    current_dir: &Path,
...
pub(in crate::product_cli) fn watch_path_is_under_dir(
    path: &Path,
    directory: &Path,
    source_root: &Path,
    current_dir: &Path,
