...
use crate::product_cli::constants::LATEST_PROTOCOL_VERSION;
use serde_json::json;
use std::{
...
pub(in crate::product_cli) fn attach_install_verification(
...
pub(in crate::product_cli) fn verify_mcp_install(
    descriptor: &NativeMcpDescriptor,
    client: &str,
) -> serde_json::Value {
...
pub(in crate::product_cli) fn verify_stdio(descriptor: &NativeMcpDescriptor) -> serde_json::Value {
...
pub(in crate::product_cli) fn verify_client_visibility(
    client: &str,
    server_name: &str,
) -> serde_json::Value {
...
pub(in crate::product_cli) fn descriptor_command(descriptor: &NativeMcpDescriptor) -> Vec<String> {
    let mut command = vec![descriptor.command.clone()];
    command.extend(descriptor.args.clone());
    command
}

pub(in crate::product_cli) fn stdio_json_rpc_message(
    method: &str,
...
pub(in crate::product_cli) fn parse_stdio_json_lines(data: &[u8]) -> Vec<serde_json::Value> {
...
pub(in crate::product_cli) fn stdio_checks(
...
pub(in crate::product_cli) fn visibility_command(client: &str) -> Option<Vec<String>> {
    match client {
        "codex" => Some(vec![
            "codex".to_string(),
            "mcp".to_string(),
