...
the archived `codebase-graph` and `okf-wiki` binary surfaces, uploads `.tar.gz` archives plus `.sha256` files, and
...
- `cargo package -p okf-wiki --locked` and the isolated Knowledge Wiki smoke
