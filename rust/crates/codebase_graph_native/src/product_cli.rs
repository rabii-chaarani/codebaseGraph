    ...
    W: Write,
{
    ...
    match args.first().map(String::as_str) {
        ...
        Some("legacy-protocol") => {
            if env::var("CODEBASE_GRAPH_ENABLE_LEGACY_PROTOCOL")
                .ok()
                .as_deref()
                == Some("1")
            {
                crate::legacy_cli::run_cli()
            } else {
                Err(
                    "legacy-protocol is test-only and is not part of the production CLI surface; \
                     set CODEBASE_GRAPH_ENABLE_LEGACY_PROTOCOL=1 only for compatibility tests"
                        .to_string(),
                )
            }
        }
...
fn run_setup<W: Write>(args: &[String], stdout: &mut W) -> Result<(), String> {
    ...
    let output = json!({
        ...
        "legacy_state_detected": source_root.join(".codebase_graph").exists(),
...
mod tests {
    ...
    fn prints_top_level_help_without_args() {
        ...
        let text = String::from_utf8(output).unwrap();
        assert!(text.contains("codebase-graph native CLI"));
        assert!(!text.contains("legacy-protocol"));
    }

    #[test]
    fn legacy_protocol_is_not_production_cli_surface() {
        let mut output = Vec::new();
        let error = run(["legacy-protocol"], &mut output).unwrap_err();
        assert!(error.contains("legacy-protocol is test-only"));
    }
